...
C  Declare all other variables.
     ...
     3   MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      INTEGER I, I1, I2, IFLAG, IMXER, KGO, LF0, COUNTI,
...
C-----------------------------------------------------------------------
c      DO COUNTI=1,137
c         IF(Y(COUNTI).GT.0.0) WRITE(*,*) "Y(",COUNTI,") = ",Y(COUNTI)
c      END DO
...
C       CHANGED THIS HERE!!!!:
c      DO COUNTI=1,137
c	      IF (Y(COUNTI).LT.1.0D-10) THEN 
c	      	Y(COUNTI) = 0.0D+0
c	      	WRITE(*,*) "Y(",COUNTI,") = ",Y(COUNTI)
c	      ENDIF
c      END DO
      GO TO (300, 300, 300), KGO
